...
namespace {
   ...
   TEST_F(VPTreeTest, empty) {
      ::testing::FLAGS_gtest_death_test_style = "threadsafe";
      Util::setShowError(false);
      vec2 lats, lons;
      EXPECT_DEATH(VPTree(lats, lons), ".*");
   }
   // One point at 3,2
   TEST_F(VPTreeTest, single) {
      vec2 lats, lons;
      std::vector<float> lat(1,3), lon(1,2);
      lats.push_back(lat);
      lons.push_back(lon);
      VPTree tree(lats, lons);
      int I, J;
   ...
   TEST_F(VPTreeTest, assignmentOperator) {
      vec2 lats, lons;
      std::vector<float> lat(1,3), lon(1,2);
      lats.push_back(lat);
      lons.push_back(lon);
   ...
   TEST_F(VPTreeTest, copyConstructor) {
      int I, J;
      vec2 lats, lons;
      std::vector<float> lat(1,3), lon(1,2);
      lats.push_back(lat);
